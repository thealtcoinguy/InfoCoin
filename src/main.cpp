...
uint256 hashGenesisBlock("0x00000af9d2ad8327724d486aa0b3866af29c8e84db9e31201aed69b672839a82");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Infocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Infocoin Signed Message:\n";
...
{
    // Infocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Infocoin: 1 day
static const int64 nTargetSpacing = 60; // Infocoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Infocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // Infocoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // Infocoin: 1 day
...
{
    // Infocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000182d3a64a9e52c6ea9f901a6f9d00875dbca9761ed2cdf07dfd4ba9e824");
    }
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "Malaysian Airlines MH370: Families struggling 6 months after jet disappeared CBCca 08092014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1410186528;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 969158;

        if (fTestNet)
        {
            block.nTime    = 1410186556;
            block.nNonce   = 0;
        }
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x0a3cf6e820b65b7edd17544246a3061b0d6ea6202c208c5e36115b58084e38a8"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Infocoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InfocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InfocoinMiner : ProcessBlock, block not accepted");
    }
